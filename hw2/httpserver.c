...
/* send file */
void http_send_file(int fd, int req_fd, struct stat *file_stat, char *file_path){
    //fprintf(stdout, "I am in send file\n");
    //fprintf(stdout, "filepath: %s fd:%d  req_fd:%d\n", file_path, fd, req_fd);
    char file_size[64];
...
/* send data , using http_send_data*/
void send_request_data(int client_fd, int req_fd) {
...
/* find index , const for security*/
int has_index(const char* path, struct stat *file_stat, char *file_path){
    //fprintf(stdout, "I am in has index!\n");
    int fd;
...
/* send directory */
void http_send_directory(int fd, char *req_dir){
    fprintf(stdout, "I am in send directory\n");
...
int tcp_connect(const char *hostname, const int server_port){
    int sock_fd, error_n;
    char serv[32];
...
void handle_proxy_request(int fd) {
  ...
  /* YOUR CODE HERE */
	  ...
	  //read from client
	  if (FD_ISSET(fd, &rset)) {
	      if ((nread = read(fd, req_buffer, MAX_BUFF)) == 0) {
		  client_eof = 1;
